...
import { logout, getUser } from '../login/auth';
...
export default function DashboardSidebar({ userRole, userName, userEmail, onCollapsedChange, children }: DashboardSidebarProps) {
  ...
  const studentMenuItems = [
    ...
    {
      href: "/frontend/main_sidebar/custom_report",
      icon: (
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${colors.border}` }}>
            {!collapsed ? (
              <div className="flex items-center space-x-3 min-w-[40px]">
                ...
                <Link href={role === 'teacher' || role === 'admin' ? "/dashboard/teacher" : "/dashboard"} 
                  className="font-bold text-xl" style={{ color: colors.text.primary }}>
                  AI知行学堂
                </Link>
              </div>
            ) : (
          ...
          <div className="p-3" style={{ borderTop: `1px solid ${colors.border}` }}>
            <ul className="space-y-1">
              <li>
                <Link 
                  href="/frontend/setting" 
                  className={`flex items-center ${collapsed ? 'justify-center' : ''} px-3 py-2.5 
                    rounded-xl transition-all duration-200`}
                  style={{ 
                    background: activeItem === '/frontend/setting' ? colors.gradient.primary : 'transparent',
                    color: activeItem === '/frontend/setting' ? '#ffffff' : colors.text.secondary,
                    boxShadow: activeItem === '/frontend/setting' ? '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' : 'none'
                  }}
                  onMouseEnter={(e) => {
                    if (activeItem !== '/frontend/setting') {
                      e.currentTarget.style.backgroundColor = colors.cardBg;
                      e.currentTarget.style.color = colors.primary;
                    }
                  }}
                ...
                >
                  <div style={{ color: activeItem === '/frontend/setting' ? '#ffffff' : 'inherit' }}>
                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
